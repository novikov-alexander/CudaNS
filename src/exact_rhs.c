    ...
    for (k = 1; k <= nz2; k++)
    {
        for (j = 1; j <= ny2; j++)
        {
            for (i = 1; i <= nx2; i++)
            {
                ...
                forcing[0][k][j][i] = forcing[0][k][j][i] - tx2 * (ue_[3][1] - ue_[1][1]) +
                                      dx1tx1 * (ue_[3][0] - 2.0 * ue_[2][0] + ue_[1][0]);

                forcing[1][k][j][i] = forcing[1][k][j][i] - tx2 * ((ue_[3][1] * buf_[3][1] + c2 * (ue_[3][4] - q_[3])) - (ue_[1][1] * buf_[1][1] + c2 * (ue_[1][4] - q_[1]))) +
                                      xxcon1 * (buf_[3][1] - 2.0 * buf_[2][1] + buf_[1][1]) +
                                      dx2tx1 * (ue_[3][1] - 2.0 * ue_[2][1] + ue_[1][1]);

                forcing[2][k][j][i] = forcing[2][k][j][i] - tx2 * (ue_[3][2] * buf_[3][1] - ue_[1][2] * buf_[1][1]) +
                                      xxcon2 * (buf_[3][2] - 2.0 * buf_[2][2] + buf_[1][2]) +
                                      dx3tx1 * (ue_[3][2] - 2.0 * ue_[2][2] + ue_[1][2]);

                forcing[3][k][j][i] = forcing[3][k][j][i] - tx2 * (ue_[3][3] * buf_[3][1] - ue_[1][3] * buf_[1][1]) +
                                      xxcon2 * (buf_[3][3] - 2.0 * buf_[2][3] + buf_[1][3]) +
                                      dx4tx1 * (ue_[3][3] - 2.0 * ue_[2][3] + ue_[1][3]);

                forcing[4][k][j][i] = forcing[4][k][j][i] - tx2 * (buf_[3][1] * (c1 * ue_[3][4] - c2 * q_[3]) - buf_[1][1] * (c1 * ue_[1][4] - c2 * q_[1])) +
                ...
                {
                    for (m = 0; m < 5; m++)
                        forcing[m][k][j][i] = forcing[m][k][j][i] - dssp * (5.0 * ue_[2][m] - 4.0 * ue_[3][m] + ue_[4][m]);
                }
                else if (i == 2)
                {
                    for (m = 0; m < 5; m++)
                        forcing[m][k][j][i] = forcing[m][k][j][i] - dssp * (-4.0 * ue_[1][m] + 6.0 * ue_[2][m] - 4.0 * ue_[3][m] + ue_[4][m]);
                }
                else if (i == nx - 3)
                {
                    for (m = 0; m < 5; m++)
                        forcing[m][k][j][i] = forcing[m][k][j][i] - dssp * (ue_[0][m] - 4.0 * ue_[1][m] + 6.0 * ue_[2][m] - 4.0 * ue_[3][m]);
                }
                else if (i == nx - 2)
                {
                    for (m = 0; m < 5; m++)
                        forcing[m][k][j][i] = forcing[m][k][j][i] - dssp * (ue_[0][m] - 4.0 * ue_[1][m] + 5.0 * ue_[2][m]);
                }
                else
                {
                    for (m = 0; m < 5; m++)
                        forcing[m][k][j][i] = forcing[m][k][j][i] - dssp * (ue_[0][m] - 4.0 * ue_[1][m] + 6.0 * ue_[2][m] - 4.0 * ue_[3][m] + ue_[4][m]);
                }
                ...
                forcing[1][k][j][i] = forcing[1][k][j][i] - ty2 * (ue_[3][1] * buf_[3][2] - ue_[1][1] * buf_[1][2]) +
                                      yycon2 * (buf_[3][1] - 2.0 * buf_[2][1] + buf_[1][1]) +
                                      dy2ty1 * (ue_[3][1] - 2.0 * ue_[2][1] + ue_[1][1]);

                forcing[2][k][j][i] = forcing[2][k][j][i] - ty2 * ((ue_[3][2] * buf_[3][2] + c2 * (ue_[3][4] - q_[3])) - (ue_[1][2] * buf_[1][2] + c2 * (ue_[1][4] - q_[1]))) +
                                      yycon1 * (buf_[3][2] - 2.0 * buf_[2][2] + buf_[1][2]) +
                                      dy3ty1 * (ue_[3][2] - 2.0 * ue_[2][2] + ue_[1][2]);

                forcing[3][k][j][i] = forcing[3][k][j][i] - ty2 * (ue_[3][3] * buf_[3][2] - ue_[1][3] * buf_[1][2]) +
                                      yycon2 * (buf_[3][3] - 2.0 * buf_[2][3] + buf_[1][3]) +
                                      dy4ty1 * (ue_[3][3] - 2.0 * ue_[2][3] + ue_[1][3]);

                forcing[4][k][j][i] = forcing[4][k][j][i] - ty2 * (buf_[3][2] * (c1 * ue_[3][4] - c2 * q_[3]) - buf_[1][2] * (c1 * ue_[1][4] - c2 * q_[1])) +
                                      0.5 * yycon3 * (buf_[3][0] - 2.0 * buf_[2][0] + buf_[1][0]) +
                ...
                forcing[1][k][j][i] = forcing[1][k][j][i] - tz2 * (ue_[3][1] * buf_[3][3] - ue_[1][1] * buf_[1][3]) +
                                      zzcon2 * (buf_[3][1] - 2.0 * buf_[2][1] + buf_[1][1]) +
                                      dz2tz1 * (ue_[3][1] - 2.0 * ue_[2][1] + ue_[1][1]);

                forcing[2][k][j][i] = forcing[2][k][j][i] - tz2 * (ue_[3][2] * buf_[3][3] - ue_[1][2] * buf_[1][3]) +
                                      zzcon2 * (buf_[3][2] - 2.0 * buf_[2][2] + buf_[1][2]) +
                                      dz3tz1 * (ue_[3][2] - 2.0 * ue_[2][2] + ue_[1][2]);

                forcing[3][k][j][i] = forcing[3][k][j][i] - tz2 * ((ue_[3][3] * buf_[3][3] + c2 * (ue_[3][4] - q_[3])) - (ue_[1][3] * buf_[1][3] + c2 * (ue_[1][4] - q_[1]))) +
                                      zzcon1 * (buf_[3][3] - 2.0 * buf_[2][3] + buf_[1][3]) +
                                      dz4tz1 * (ue_[3][3] - 2.0 * ue_[2][3] + ue_[1][3]);

                forcing[4][k][j][i] = forcing[4][k][j][i] - tz2 * (buf_[3][3] * (c1 * ue_[3][4] - c2 * q_[3]) - buf_[1][3] * (c1 * ue_[1][4] - c2 * q_[1])) +
